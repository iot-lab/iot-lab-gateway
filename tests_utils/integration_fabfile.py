...
def download():
    ...
    extra_opts = (
        " --exclude='.tox' --exclude='*egg*'" " --include='*/'" " --include='*xml'" " --exclude='*'"
    )
    rsync_project(
        remote_dir=REMOTE + "/",
        local_dir=LOCAL,
...
def kill():
    ...
    with settings(warn_only=True):
        run("killall" " python socat control_node_serial_interface openocd" " || true")


def safe_su(command, user="root"):
    """Run a 'warn_only' su with command"""
    with settings(warn_only=True):
        return run('su {user} -c "source /etc/profile; {cmd}"'.format(user=user, cmd=command))
