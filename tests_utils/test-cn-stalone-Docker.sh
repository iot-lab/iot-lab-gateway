...
# Rebuild the control node serial interface
docker exec gateway_test 'python' 'setup.py' 'build_ext'
docker exec gateway_test 'cp' 'control_node_serial/control_node_serial_interface' '/usr/local/bin/'
...
output_file="/iotlab/users/$user/.iot-lab/$experiment/sniffer/custom_$experiment.oml"
