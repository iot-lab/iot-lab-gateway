...
ANSWERS = {
    ...
    # 3.3V  vc 1100 256
    get_str([0x79, 0x16, 0xd4]): [[0x79, 0xA], [0xFA, 0x79, 0x16]],
    # start/stop DC
...
MEASURES = {
    get_str([0x79]): {
        ...
        get_str([0x17, 0xe4]):
        [0xFF, 0x01, 0xF6, 0x10, 0x01, 0x00, 0x75, 0x82, 0x84, 0x3E, 0x3D,
         0x0A, 0x4F, 0x40, 0xCC, 0xD8, 0xA3, 0x3D],
        get_str([0x12, 0xe4]):
        [0xFF, 0x01, 0xE9, 0x91, 0x03, 0x00, 0x3D, 0x0A, 0x4F, 0x40],
        get_str([0x16, 0xd4]):
        [0xFF, 0x01, 0xF5, 0x54, 0x00, 0x00, 0xB8, 0x1E, 0x4F, 0x40, 0xCC,
         0xD8, 0xA3, 0x3D],
    }
}
...
class MeasuresState(object):
    ...
    def update_measures(self, str_payload):
        meas_t = str_payload[0]
        try:
            self.measures_dict[meas_t] = MEASURES[meas_t][str_payload[1:]]
        except KeyError:
